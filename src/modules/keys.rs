...
use crate::clients::compositor::KeyboardLayoutUpdate;
...
struct Icons {
    ...
    /// Map of icons to show for a particular keyboard layout.
    ///
    ...
    fn spawn_controller(
        ...
        spawn(async move {
            ...
            while let Ok(ev) = rx.recv().await {
                match ev {
                    ...
                    Event::Key(ev) => {
                        send_async!(tx, ModuleUpdateEvent::Update(KeysUpdate::Key(ev)));
                    }
                }
            }
        });

        let client = context.ironbar.clients.borrow_mut().keyboard_layout()?;
        {
            let client = client.clone();
            let tx = context.tx.clone();
            spawn(async move {
                let mut srx = client.subscribe_keyboard_layout_change();

                trace!("Set up keyboard_layout subscription");

                while let Ok(payload) = srx.recv().await {
                    debug!("Received update: {payload:?}");
                    send_async!(tx, ModuleUpdateEvent::Update(KeysUpdate::Layout(payload)));
                }
            });
        }
        ...
        spawn(async move {
            ...
            while let Some(()) = rx.recv().await {
                if let Err(e) = client.next_keyboard_layout() {
                    warn!("Couldn't change keyboard layout: {e:#}");
                };
            }
