...
impl Module<Button> for BluetoothModule {
    ...
    fn into_popup(
        ...
        let vscrollbar_policy = match self.popup.max_height {
            Some(SizeLimit::Pixel(max_height)) if max_height > 0 => {
                devices_window.set_max_content_height(max_height);
                gtk::PolicyType::Automatic
            }
            Some(SizeLimit::Count(_)) => gtk::PolicyType::Automatic,
            Some(SizeLimit::Pixel(_)) | None => gtk::PolicyType::Never,
        };
...

        {
            ...
            rx.recv_glib((), move |(), state: BluetoothState| {
                ...
                if let BluetoothState::Enabled { devices } = state {
                    ...
                    if let Some(SizeLimit::Count(max_dev_count)) = max_height {
