...
use gtk::{
    Box as GtkBox, GestureClick, Orientation, Shortcut, ShortcutAction, ShortcutController,
...
impl TrayMenu {
    pub fn new(
        ...
        gesture.connect_pressed(move |gesture, _, _, _| {
            ...
            spawn(async move {
                tx.send(ActivateRequest::Default {
        ...
        gesture.connect_pressed(move |gesture, _, _, _| {
            ...
            spawn(async move {
                tx.send(ActivateRequest::Secondary {
                    address: address.to_owned(),
                    x: 0,
                    y: 0,
                })
...
// FIXME: this never fires most of the time. god knows why
fn activate(tx: &mpsc::Sender<ActivateRequest>, address: &str, path: &str, id: i32) {
    println!("HELLO I AM ACTIVATED");
