...
pub fn install(path: &Path, app: &Application, ironbar: &Rc<Ironbar>) {
    ...
    rx.recv_glib(app, move |app, ()| {
        ...
        if matches!(diff.icon_theme, Diff::Updated(()))
            || matches!(diff.icon_overrides, Diff::Updated(()))
        {
            let provider = ironbar.image_provider();
            provider.set_icon_theme(new_config.borrow().icon_theme.as_deref());
            provider.set_overrides(&mut new_config.borrow_mut().icon_overrides);

            info!("Icon settings changed, reload required");
            Ironbar::reload(&ironbar, app);
            return;
        }
