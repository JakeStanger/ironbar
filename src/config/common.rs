...
use gtk::gdk::{ScrollDirection, BUTTON_MIDDLE, BUTTON_PRIMARY, BUTTON_SECONDARY};
use gtk::prelude::*;
use gtk::{EventControllerMotion, EventControllerScroll, EventControllerScrollFlags, GestureClick, Justification, Orientation, Revealer, RevealerTransitionType, Widget};
...
impl CommonConfig {
        ...
        W: IsA<Widget>,
    {
        ...
        if let Some(script) = self.on_mouse_enter.map(Script::new_polling) { event_controller.connect_enter(move |_, _, _| {
                script.run_as_oneshot(None);
            }); }

        if let Some(script) = self.on_mouse_exit.map(Script::new_polling) { event_controller.connect_leave(move |_| {
                script.run_as_oneshot(None);
            }); }
