...
use crate::{error, register_fallible_client, spawn, Ironbar};
...
pub struct BluetoothDevice {
    ...
    /// Indicates if the remote device connection status
...
impl Client {
            ...
            spawn(async move {
                ...
                loop {
                    match Self::get_state(&session).await {
                        Ok(mut new_state) => {
                            info!("New bluetooth state: {:?}", new_state);
                ...
                info!("Stopping bluetooth session");
            });
        }
    ...
    pub(crate) fn disconnect(&self, address: bluer::Address) {
        ...
        spawn(async move {
            ...
            let new_status = if let Err(err) = Self::disconnect_device(session, address).await {
                ...
                info!("Disconnect finished {}", address);
                BluetoothDeviceStatus::Disconnected
            };
    ...
    /// These changes are transiet states of adaptor and devices like `Connecting`, `Enabling`, etc
