...
| `format`       | `string` or `object`                        | `"{label}"` | The format string for the workspace button. Can be a string to apply to all, or an object with `named` and `unnamed` keys. See [formatting](#formatting) for more info.   |
